...
export class FileEncryptionUseCase {
      ...
      for (const keyPath of possiblePaths) {
        try {
          if (fs.existsSync(keyPath)) {
            ...
            keyLoaded = true;
            console.log(`[+] Llave publica cargada desde: ${keyPath}`);
            break;
          }
        } catch (error) {
  ...
  private async encryptFile(filePath: string, outputFilePath: string): Promise<void> {
    try {
      ...
      console.log(`[*] Archivo cifrado: ${outputFilePath}`);
    } catch (error) {
      console.error(`[!] Error al cifrar el archivo ${filePath}:`, error);
      throw new Error('Fallo en el cifrado del archivo');
    }
  }

  // Método para cifrar todos los archivos dentro de una carpeta (recursivamente)
  async encryptAllFilesInFolder(folderPath: string, outputFolderPath: string): Promise<void> {
    try {
      ...
      console.log(`[+] Procesada carpeta: ${folderPath}`);
    } catch (error) {
